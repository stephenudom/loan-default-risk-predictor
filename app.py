...
if "Customer Application" in view_mode:
    ...
    if st.session_state.customer_submitted:
        ...
        st.markdown('<div class="section-header">4. Supporting Documents</div>', unsafe_allow_html=True)
        st.markdown("""
        <div style='background:linear-gradient(135deg,rgba(0,212,255,0.06),rgba(0,212,255,0.02));
                    border:1px solid rgba(0,212,255,0.15);border-radius:8px;padding:12px 18px;
                    font-size:12px;color:#C8D8E8;margin-bottom:16px;line-height:1.8;font-family:Sora,sans-serif'>
            Upload clear, legible copies of the documents below. Accepted formats: PDF, JPG, PNG.
        ...
        # ── SECTION 5: BVN VERIFICATION ──────────
        st.markdown('<div class="section-header">5. BVN Verification (Required)</div>', unsafe_allow_html=True)
        st.markdown("""
